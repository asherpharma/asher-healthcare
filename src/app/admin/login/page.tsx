  ...
  return (
    <main id="main-content" className="min-h-screen bg-slate-50 px-6 py-10 sm:py-16">
      <div className="mx-auto max-w-lg">
        <a href="/" className="inline-flex items-center gap-2 text-sm font-bold text-[#233A59] hover:text-[#A8864A]"><ArrowLeft size={16} />Back to website</a>
        <section className="mt-8 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-xl shadow-slate-200/60 sm:p-10">
          ...
          <h1 className="mt-3 text-3xl font-bold tracking-tight text-[#233A59]">{welcome ? "Your staff account is ready." : "Welcome back."}</h1>
          ...
          {!isFirebaseConfigured ? (
            ...
            <>
              ...
              <form className="mt-7 space-y-5" onSubmit={signIn}>
                ...
                <button disabled={loading} className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#233A59] px-5 text-sm font-bold text-white transition hover:bg-[#1b2e48] disabled:opacity-60">{loading ? <LoaderCircle className="animate-spin" size={18} /> : <KeyRound size={18} />}{loading ? "Signing in…" : "Sign in securely"}</button>
              </form>
                ...
                {!resetOpen ? (
                  ...
                  <form onSubmit={requestPasswordReset} className="rounded-2xl bg-slate-50 p-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div><h2 className="font-bold text-[#233A59]">Reset your password</h2><p className="mt-1 text-sm leading-6 text-slate-600">We’ll email a secure reset link to your approved staff address.</p></div>
                    </div>
                    ...
                    <div className="mt-4 grid gap-2 sm:grid-cols-2">
                      <button type="submit" disabled={resetting} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#A8864A] px-4 text-sm font-bold text-white disabled:opacity-60">{resetting ? <LoaderCircle className="animate-spin" size={17} /> : <Mail size={17} />}{resetting ? "Sending…" : "Send reset link"}</button>
          ...
          <p className="mt-4 text-xs leading-5 text-slate-500">New staff receive one private password-setup email. Normal sign-in uses email and password, so no SMS charge is created.</p>
        </section>
      </div>
    </main>
  );
