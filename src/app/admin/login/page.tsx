...
import StaffPasswordlessLogin from "@/components/admin/StaffPasswordlessLogin";
import { firebaseAuth, isFirebaseConfigured } from "@/firebase/config";
import { sendPasswordResetEmail, signInWithEmailAndPassword } from "firebase/auth";
import { ArrowLeft, CheckCircle2, ChevronDown, KeyRound, LoaderCircle, LockKeyhole, Mail, ShieldCheck, Smartphone } from "lucide-react";
    ...
    <main id="main-content" className="min-h-screen bg-slate-50 px-6 py-10 sm:py-16">
      <div className="mx-auto max-w-lg">
        ...
        <section className="mt-8 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-xl shadow-slate-200/60 sm:p-10">
          ...
          <p className="mt-3 leading-7 text-slate-600">{welcome ? "Use the approved email from your invitation. You can install the app and link your mobile for faster OTP access after signing in." : "Choose the quickest secure method for your clinic-approved staff account."}</p>
          {welcome ? (
          ...
          {!isFirebaseConfigured ? (
            ...
            <>
              ...
              {message && <p role="alert" className="mt-6 rounded-xl bg-red-50 px-4 py-3 text-sm text-red-700">{message}</p>}
              <StaffPasswordlessLogin initialEmail={initialEmail} afterSignInHref={welcome ? "/admin/app" : "/admin"} />
              <div className="mt-5 border-t border-slate-200 pt-5">
                <button type="button" aria-expanded={passwordOpen} onClick={() => { setPasswordOpen((open) => !open); setResetOpen(false); setMessage(""); }} className="flex min-h-11 w-full items-center justify-between gap-3 text-left text-sm font-bold text-slate-600 hover:text-[#233A59]">
                  <span className="inline-flex items-center gap-2"><KeyRound size={17} />Use email and password instead</span><ChevronDown size={17} className={`transition ${passwordOpen ? "rotate-180" : ""}`} />
                </button>
                {passwordOpen ? (
                  <form className="mt-4 space-y-5 rounded-2xl bg-slate-50 p-4" onSubmit={signIn}>
                    ...
                    <label className="block text-sm font-bold text-slate-700">Password<input name="password" type="password" autoComplete="current-password" required minLength={8} className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 outline-none transition focus:border-[#233A59] focus:ring-2 focus:ring-[#233A59]/15" /></label>
          ...
          {isFirebaseConfigured && passwordOpen ? (
            <div className="mt-5 border-t border-slate-200 pt-5">
              {!resetOpen ? (
          ...
          <p className="mt-4 text-xs leading-5 text-slate-500">Email links and mobile OTP remain restricted to active staff. Link your own number once from <strong>Mobile app</strong>; the administrator never receives your OTP.</p>
        </section>
      </div>
    </main>
  );
