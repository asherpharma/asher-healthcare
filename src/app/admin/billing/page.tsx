    ...
    <div>
        ...
        <section className="mt-6 rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200 sm:p-7">
          ...
          <form onSubmit={createInvoice} className="mt-6 space-y-6">
            ...
            <div className="grid gap-4 rounded-2xl border border-slate-200 p-4 md:grid-cols-2 xl:grid-cols-4">
              ...
              <label className={labelClass}>Payment method<select value={paymentMethod} onChange={(event) => setPaymentMethod(event.target.value as PaymentMethod)} className={inputClass}><option value="cash">Cash</option><option value="upi">UPI</option><option value="card">Card</option><option value="bank_transfer">Bank transfer</option><option value="online">Online gateway</option></select></label>
      ...
      {payingInvoice && (
        <section className="mt-6 rounded-3xl border border-emerald-200 bg-emerald-50 p-5 sm:p-7">
          <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-emerald-700">Record payment</p><h2 className="mt-2 text-xl font-bold text-[#233A59]">{payingInvoice.invoiceNumber} · {payingInvoice.patientName}</h2><p className="mt-1 text-sm text-slate-600">Outstanding balance: {money(payingInvoice.balance)}</p></div><button type="button" onClick={() => setPayingInvoice(null)} aria-label="Close payment form" className="rounded-lg p-2 text-slate-500 hover:bg-white"><X size={19} /></button></div>
          <form onSubmit={recordPayment} className="mt-5 grid gap-4 md:grid-cols-[1fr_1fr_1.2fr_auto] md:items-end">
            ...
            <label className={labelClass}>Method<select value={followupMethod} onChange={(event) => setFollowupMethod(event.target.value as PaymentMethod)} className={inputClass}><option value="cash">Cash</option><option value="upi">UPI</option><option value="card">Card</option><option value="bank_transfer">Bank transfer</option><option value="online">Online gateway</option></select></label>
            ...
            <button type="submit" disabled={recordingPayment} className="inline-flex h-[46px] items-center justify-center gap-2 rounded-xl bg-emerald-700 px-5 text-sm font-bold text-white disabled:opacity-60">{recordingPayment ? <LoaderCircle size={17} className="animate-spin" /> : <CheckCircle2 size={17} />} Receive</button>
          </form>
        </section>
      )}
